...
internal static class EdgeRouter
{
		...
		IReadOnlyList<LayoutEdge>? inputEdges = null)
	{
		...
		{
			var points = reversed && chain[0] < graph.RealNodeCount && chain[chain.Count - 1] < graph.RealNodeCount
				? RouteBackEdge(graph, chain[0], chain[chain.Count - 1])
				: RouteChain(graph, chain, useSideRouting);

			var src = chain[0];
			var tgt = chain[chain.Count - 1];
		...
		int node, double cx, double cy, bool isReal, double sourceCX = double.NaN)
	{
		...
		{
			if (points.Count > 0)
				points[points.Count - 1] = new LayoutPoint(cx, points[points.Count - 1].Y);
