...
internal sealed partial record FontScale
{
	...
	private static (double Value, string Unit) ParseSize(string size)
	{
		...
		if (!match.Success
			...
			|| !Mermaider.Compatibility.Net48Compat.IsFinite(value))
			return (1, "rem");
	...
	private static double NormalizeRatio(double? candidate, double fallback) =>
		candidate is > 0 && Mermaider.Compatibility.Net48Compat.IsFinite(candidate.Value) ? candidate.Value : fallback;

	private static string FormatSize(double value, string unit) =>
		$"{value.ToString("0.###", CultureInfo.InvariantCulture)}{unit}";

	private static Regex SizePattern() => Net48SizePatternRegexCache.Value;
	private static class Net48SizePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^((?:0|[1-9][0-9]*)(?:\.[0-9]{1,3})?)\s*(px|rem|em|%)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(2000));
	}
}
