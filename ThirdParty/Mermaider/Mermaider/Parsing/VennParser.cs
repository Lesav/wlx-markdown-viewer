...
internal static partial class VennParser
{
	...
	private static Regex SetPattern() => Net48SetPatternRegexCache.Value;
	private static class Net48SetPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^set\s+(?:""([^""]+)""|([^\s\[]+))(?:\s*\[""([^""]+)""\])?\s*(?::\s*(\d+(?:\.\d+)?))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex UnionPattern() => Net48UnionPatternRegexCache.Value;
	private static class Net48UnionPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^union\s+(.+?)\s*(?:\[""([^""]+)""\])?\s*(?::\s*(\d+(?:\.\d+)?))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
