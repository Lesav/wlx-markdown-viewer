...
internal static partial class BlockParser
{
	...
	{
		internal static readonly Regex Value = new Regex(@"^block(?:-beta)?(?:\s+title\s+(.+))?$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex ColumnsPattern() => Net48ColumnsPatternRegexCache.Value;
	private static class Net48ColumnsPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^columns\s+(\d+)\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex EdgePattern() => Net48EdgePatternRegexCache.Value;
	private static class Net48EdgePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^([A-Za-z_][\w-]*)\s*-->\s*([A-Za-z_][\w-]*)\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
