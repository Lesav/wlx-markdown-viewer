...
internal static partial class StrictStylingValidator
{
	...
	private static Regex ClassDefDirective() => Net48ClassDefDirectiveRegexCache.Value;
	private static class Net48ClassDefDirectiveRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^classDef\s+", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StyleDirective() => Net48StyleDirectiveRegexCache.Value;
	private static class Net48StyleDirectiveRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^style\s+", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex LinkStyleDirective() => Net48LinkStyleDirectiveRegexCache.Value;
	private static class Net48LinkStyleDirectiveRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^linkStyle\b", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex UpdateStyleDirective() => Net48UpdateStyleDirectiveRegexCache.Value;
	private static class Net48UpdateStyleDirectiveRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^Update\w*Style\b", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex ClassAssignDirective() => Net48ClassAssignDirectiveRegexCache.Value;
	private static class Net48ClassAssignDirectiveRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^class\s+[\w,-]+\s+(\w+)\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex ClassShorthand() => Net48ClassShorthandRegexCache.Value;
	private static class Net48ClassShorthandRegexCache
	{
	...
	private static Regex SafeClassName() => Net48SafeClassNameRegexCache.Value;
	private static class Net48SafeClassNameRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^[A-Za-z_][A-Za-z0-9_-]*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static void ValidateClassShorthand(string line, FrozenSet<string> allowed, int lineIndex,
		StrictStylingOptions strict, List<StrictStylingViolation> violations)
	{
		foreach (Match match in ClassShorthand().Matches(line))
		{
