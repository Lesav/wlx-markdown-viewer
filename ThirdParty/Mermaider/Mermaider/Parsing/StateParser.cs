...
internal static partial class StateParser
{
	...
	private static Regex CompositeStatePattern() => Net48CompositeStatePatternRegexCache.Value;
	private static class Net48CompositeStatePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^state\s+(?:""([^""]+)""\s+as\s+)?(\w+)\s*\{$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StateAliasPattern() => Net48StateAliasPatternRegexCache.Value;
	private static class Net48StateAliasPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^state\s+""([^""]+)""\s+as\s+(\w+)\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StateDescPattern() => Net48StateDescPatternRegexCache.Value;
	private static class Net48StateDescPatternRegexCache
	{
	...
	private static Regex StereotypePattern() => Net48StereotypePatternRegexCache.Value;
	private static class Net48StereotypePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^state\s+(\w+)\s+<<(choice|fork|join)>>\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex NoteInlinePattern() => Net48NoteInlinePatternRegexCache.Value;
	private static class Net48NoteInlinePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^note\s+(left|right)\s+of\s+(\w[\w-]*)\s*:\s*(.+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex NoteBlockStartPattern() => Net48NoteBlockStartPatternRegexCache.Value;
	private static class Net48NoteBlockStartPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^note\s+(left|right)\s+of\s+(\w[\w-]*)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex TransitionWithClassPattern() => Net48TransitionWithClassPatternRegexCache.Value;
	private static class Net48TransitionWithClassPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\[\*\]|[\w-]+)(?::::([\w][\w-]*))?\s*(-->)\s*(\[\*\]|[\w-]+)(?::::([\w][\w-]*))?(?:\s*:\s*(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	{
		internal static readonly Regex Value = new Regex(@"^([\w-]+):::([\w][\w-]*)\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static MermaidGraph ParseCore(string[] lines)
	{
		...
		for (var i = 1; i < lines.Length; i++)
		{
			...
			if (dirMatch.Success)
			{
				var dir = Mermaider.Compatibility.Net48Compat.ParseEnum<Direction>(dirMatch.Groups[1].Value.ToUpperInvariant());
