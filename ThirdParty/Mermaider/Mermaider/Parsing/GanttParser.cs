...
internal static partial class GanttParser
{
	...
	{
		internal static readonly Regex Value = new Regex(@"^gantt(?:\s+title\s+(.+))?\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex DateFormatPattern() => Net48DateFormatPatternRegexCache.Value;
	private static class Net48DateFormatPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^dateFormat\s+(.+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	/// </summary>
	private static Regex DurationPattern() => Net48DurationPatternRegexCache.Value;
	private static class Net48DurationPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^\d+(?:\.\d+)?(?:ms|[smhdwyM])$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex AfterPattern() => Net48AfterPatternRegexCache.Value;
	private static class Net48AfterPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^after\s+(.+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex ExcludesPattern() => Net48ExcludesPatternRegexCache.Value;
	private static class Net48ExcludesPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^excludes\s+(.+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static string[] ParseAfterIds(string token)
	{
		...
		if (!m.Success)
			return [];
		return Mermaider.Compatibility.Net48Compat.SplitAndTrim(
			m.Groups[1].Value, (char[])null, true);
	}
	...
	private static ParsedDuration ParseDuration(string token)
	{
		...
		if (!double.TryParse(token.Substring(0, numberLen), NumberStyles.Float, CultureInfo.InvariantCulture, out var number))
