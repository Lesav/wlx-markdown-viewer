...
internal static partial class PacketParser
{
	...
	{
		internal static readonly Regex Value = new Regex(@"^packet(?:-beta)?(?:\s+title\s+(.+))?$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex FieldPattern() => Net48FieldPatternRegexCache.Value;
	private static class Net48FieldPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(?:\+(\d+)|(\d+)(?:-(\d+))?)\s*:\s*""([^""]*)""\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
