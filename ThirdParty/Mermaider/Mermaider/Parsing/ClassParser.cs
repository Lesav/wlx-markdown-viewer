...
internal static partial class ClassParser
{
	...
	private static Regex NamespaceStartPattern() => Net48NamespaceStartPatternRegexCache.Value;
	private static class Net48NamespaceStartPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^namespace\s+(\S+)\s*\{$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex ClassBlockPattern() => Net48ClassBlockPatternRegexCache.Value;
	private static class Net48ClassBlockPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^class\s+(\S+?)(?:\s*~(\w+)~)?\s*\{$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex ClassOnlyPattern() => Net48ClassOnlyPatternRegexCache.Value;
	private static class Net48ClassOnlyPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^class\s+(\S+?)(?:\s*~(\w+)~)?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex InlineAnnotationPattern() => Net48InlineAnnotationPatternRegexCache.Value;
	private static class Net48InlineAnnotationPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^class\s+(\S+?)\s*\{\s*<<(\w+)>>\s*\}$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex AnnotationPattern() => Net48AnnotationPatternRegexCache.Value;
	private static class Net48AnnotationPatternRegexCache
	{
	...
	private static Regex InlineAttrPattern() => Net48InlineAttrPatternRegexCache.Value;
	private static class Net48InlineAttrPatternRegexCache
	{
	...
	private static Regex ArrowCheckPattern() => Net48ArrowCheckPatternRegexCache.Value;
	private static class Net48ArrowCheckPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"<\|--|--|\*--|o--|-->|\.\.>|\.\.\|>", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex RelationshipPattern() => Net48RelationshipPatternRegexCache.Value;
	private static class Net48RelationshipPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\S+?)\s+(?:""([^""]*?)""\s+)?(<\|--|<\|\.\.|\*--|o--|-->|--\*|--o|--\|>|\.\.>|\.\.\|>|<--|<\.\.?|--)\s+(?:""([^""]*?)""\s+)?(\S+?)(?:\s*:\s*(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex VisibilityPrefixPattern() => Net48VisibilityPrefixPatternRegexCache.Value;
	private static class Net48VisibilityPrefixPatternRegexCache
	{
	...
	private static Regex MethodSignaturePattern() => Net48MethodSignaturePatternRegexCache.Value;
	private static class Net48MethodSignaturePatternRegexCache
	{
	...
	private static Regex NotePattern() => Net48NotePatternRegexCache.Value;
	private static class Net48NotePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^note\s+(?:for\s+(\S+)\s+)?""([^""]+)""$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex LollipopPattern() => Net48LollipopPatternRegexCache.Value;
	private static class Net48LollipopPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\S+?)\s+(--\(\)|\.\.?\(\)|\(\)--|\.?\(\)\.\.)\s+(\S+?)(?:\s*:\s*(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static ClassDiagram ParseCore(string[] lines)
	{
		...
		for (var i = 1; i < lines.Length; i++)
		{
			...
			if (dirMatch.Success)
			{
				direction = Mermaider.Compatibility.Net48Compat.ParseEnum<Direction>(dirMatch.Groups[1].Value.ToUpperInvariant());
				continue;
			}
	...
	private static (ClassMember Member, bool IsMethod)? ParseMember(string line)
	{
		...
		{
			memberType = parts[0];
			memberName = string.Join(" ", parts.Skip(1));
		}
		else
		{
