...
internal static partial class KanbanParser
{
	...
	private static Regex IdTitlePattern() => Net48IdTitlePatternRegexCache.Value;
	private static class Net48IdTitlePatternRegexCache
	{
	...
	private static Regex MetadataSuffixPattern() => Net48MetadataSuffixPatternRegexCache.Value;
	private static class Net48MetadataSuffixPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(.+?)\s*@\{([^}]*)\}\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex MetadataPairPattern() => Net48MetadataPairPatternRegexCache.Value;
	private static class Net48MetadataPairPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"(\w+)\s*:\s*(?:'([^']*)'|""([^""]*)""|([^,}\s][^,}]*)?)", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
