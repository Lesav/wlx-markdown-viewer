...
internal static partial class ArchitectureParser
{
	...
	private static Regex GroupPattern() => Net48GroupPatternRegexCache.Value;
	private static class Net48GroupPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^group\s+(\S+?)(?:\(([^)]*)\))?\s*\[([^\]]*)\](?:\s+in\s+(\S+))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex ServicePattern() => Net48ServicePatternRegexCache.Value;
	private static class Net48ServicePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^service\s+(\S+?)(?:\(([^)]*)\))?\s*\[([^\]]*)\](?:\s+in\s+(\S+))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex JunctionPattern() => Net48JunctionPatternRegexCache.Value;
	private static class Net48JunctionPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^junction\s+(\S+?)(?:\s+in\s+(\S+))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	// A(:side)? (--|-->|<--|<-->) (side:)?B — braces around group endpoints are stripped before matching.
	private static Regex EdgePattern() => Net48EdgePatternRegexCache.Value;
	private static class Net48EdgePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\S+?)(?::(L|R|T|B))?\s*(--|-->|<--|<-->)\s*(?:(L|R|T|B):)?(\S+)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
