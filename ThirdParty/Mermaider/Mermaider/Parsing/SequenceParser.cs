...
internal static partial class SequenceParser
{
	...
	private static Regex ActorPattern() => Net48ActorPatternRegexCache.Value;
	private static class Net48ActorPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(participant|actor)\s+(\S+?)(?:\s+as\s+(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex NotePattern() => Net48NotePatternRegexCache.Value;
	private static class Net48NotePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^Note\s+(left of|right of|over)\s+([^:]+):\s*(.+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex BlockStartPattern() => Net48BlockStartPatternRegexCache.Value;
	private static class Net48BlockStartPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(loop|alt|opt|par|critical|break|rect)\s*(.*)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex DividerPattern() => Net48DividerPatternRegexCache.Value;
	private static class Net48DividerPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(else|and)\s*(.*)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex MessagePattern() => Net48MessagePatternRegexCache.Value;
	private static class Net48MessagePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\S+?)\s*(->>|-->>|-\)|--\)|-x|--x|->|-->)\s*([+-]?)(\S+?)\s*:\s*(.+)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex AutonumberPattern() => Net48AutonumberPatternRegexCache.Value;
	private static class Net48AutonumberPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^autonumber(?:\s+(\d+))?(?:\s+(\d+))?\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex BidirectionalMessagePattern() => Net48BidirectionalMessagePatternRegexCache.Value;
	private static class Net48BidirectionalMessagePatternRegexCache
	{
	...
	private static Regex BoxPattern() => Net48BoxPatternRegexCache.Value;
	private static class Net48BoxPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^box(?:\s+(\S+))?(?:\s+(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex CreatePattern() => Net48CreatePatternRegexCache.Value;
	private static class Net48CreatePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^create\s+(participant|actor)\s+(\S+?)(?:\s+as\s+(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex DestroyPattern() => Net48DestroyPatternRegexCache.Value;
	private static class Net48DestroyPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^destroy\s+(\S+)\s*$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static SequenceDiagram ParseCore(string[] lines)
	{
		...
		for (var i = 1; i < lines.Length; i++)
		{
			...
			if (noteMatch.Success)
			{
				...
				var noteActorIds = Mermaider.Compatibility.Net48Compat.SplitAndTrim(actorsStr, new[] { ',' }, false);
			...
			{
				var blockType = Mermaider.Compatibility.Net48Compat.ParseEnum<SequenceBlockType>(blockMatch.Groups[1].Value, ignoreCase: true);
