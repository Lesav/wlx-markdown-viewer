...
internal static partial class XyChartParser
{
	...
	{
		internal static readonly Regex Value = new Regex(@"^xychart(?:-beta)?(?:\s+(horizontal|vertical))?(?:\s+title\s+(.+))?$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	{
		internal static readonly Regex Value = new Regex(@"^x-axis\s+(.+)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex YAxisPattern() => Net48YAxisPatternRegexCache.Value;
	private static class Net48YAxisPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^y-axis\s+(.+)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex SeriesPattern() => Net48SeriesPatternRegexCache.Value;
	private static class Net48SeriesPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(bar|line)(?:\s+(.+?))?\s*\[(.+)\]\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
