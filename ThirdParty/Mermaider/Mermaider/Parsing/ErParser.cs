...
internal static partial class ErParser
{
	...
	private static Regex EntityBlockPattern() => Net48EntityBlockPatternRegexCache.Value;
	private static class Net48EntityBlockPatternRegexCache
	{
	...
	private static Regex AttributePattern() => Net48AttributePatternRegexCache.Value;
	private static class Net48AttributePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(\S+)\s+(\S+)(?:\s+(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex CommentPattern() => Net48CommentPatternRegexCache.Value;
	private static class Net48CommentPatternRegexCache
	{
	...
	{
		internal static readonly Regex Value = new Regex(@"^(\S+)\s+([|o}{]+(?:--|\.\.)[|o}{]+)\s+(\S+)(?:\s*:\s*(.+))?$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex CardinalitySplitPattern() => Net48CardinalitySplitPatternRegexCache.Value;
	private static class Net48CardinalitySplitPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^([|o}{]+)(--|\.\.)([|o}{]+)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
