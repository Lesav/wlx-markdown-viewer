...
internal static partial class MindmapParser
{
	...
	private static Regex CircleShape() => Net48CircleShapeRegexCache.Value;
	private static class Net48CircleShapeRegexCache
	{
	...
	private static Regex SquareShape() => Net48SquareShapeRegexCache.Value;
	private static class Net48SquareShapeRegexCache
	{
		internal static readonly Regex Value = new Regex(@"\[(.+?)\]$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex RoundedShape() => Net48RoundedShapeRegexCache.Value;
	private static class Net48RoundedShapeRegexCache
	{
		internal static readonly Regex Value = new Regex(@"(?<!\()\(([^()]+?)\)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex CloudShape() => Net48CloudShapeRegexCache.Value;
	private static class Net48CloudShapeRegexCache
	{
		internal static readonly Regex Value = new Regex(@"\)\)(.+?)\(\($", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex HexagonShape() => Net48HexagonShapeRegexCache.Value;
	private static class Net48HexagonShapeRegexCache
	{
		internal static readonly Regex Value = new Regex(@"\{\{(.+?)\}\}$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex BangShape() => Net48BangShapeRegexCache.Value;
	private static class Net48BangShapeRegexCache
	{
		internal static readonly Regex Value = new Regex(@"(?<!\))\)(.+?)\($", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
