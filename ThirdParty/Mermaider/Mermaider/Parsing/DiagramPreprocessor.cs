...
internal static partial class DiagramPreprocessor
{
	...
	private static Regex InitDirectiveRegex() => Net48InitDirectiveRegexRegexCache.Value;
	private static class Net48InitDirectiveRegexRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^%%\{init:\s*(\{.*\})\s*\}%%\s*$", RegexOptions.Singleline | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
		...
		foreach (var line in lines)
		{
			...
			if (match.Success)
			{
				...
				try
				{
					...
					if (root is not null)
					{
						if (root.TryGetValue("theme", out var themeValue) && themeValue is string parsedTheme)
							theme = parsedTheme;

						if (root.TryGetValue("themeVariables", out var variablesValue) &&
							variablesValue is Dictionary<string, object> variables)
						{
							themeVariables ??= [];
							foreach (var pair in variables)
							{
								if (pair.Value is string value)
									themeVariables[pair.Key] = value;
							}
						}

						if (root.TryGetValue("title", out var titleValue) && titleValue is string parsedTitle)
							title = parsedTitle;
					}
				}
