...
internal static partial class C4Parser
{
	...
	{
		internal static readonly Regex Value = new Regex(@"^C4(Context|Container|Component|Dynamic|Deployment)\b(?:\s+title\s+(.+))?$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
	...
	private static Regex ElementOrBoundaryPattern() => Net48ElementOrBoundaryPatternRegexCache.Value;
	private static class Net48ElementOrBoundaryPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(Person(?:_Ext)?|System(?:Db|Queue)?(?:_Ext)?|Container(?:Db|Queue)?(?:_Ext)?|Component(?:Db|Queue)?(?:_Ext)?|Deployment_Node|Node(?:_[LR])?|Boundary|Enterprise_Boundary|System_Boundary|Container_Boundary)\s*\((.+)\)\s*(\{)?\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex RelationPattern() => Net48RelationPatternRegexCache.Value;
	private static class Net48RelationPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(BiRel|Rel_Back|Rel_U|Rel_Up|Rel_D|Rel_Down|Rel_L|Rel_Left|Rel_R|Rel_Right|RelIndex|Rel)\s*\((.+)\)\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex LayoutConfigPattern() => Net48LayoutConfigPatternRegexCache.Value;
	private static class Net48LayoutConfigPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^UpdateLayoutConfig\s*\((.+)\)\s*$", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StyleUpdatePattern() => Net48StyleUpdatePatternRegexCache.Value;
	private static class Net48StyleUpdatePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^Update(?:Element|Rel)Style\s*\(", RegexOptions.IgnoreCase | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
