...
public static partial class SvgSanitizer
{
	...
	/// </summary>
	private static Regex SafeImageDataUriPattern() => Net48SafeImageDataUriPatternRegexCache.Value;
	private static class Net48SafeImageDataUriPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^data:image/(?<type>svg\+xml|png);base64,(?<payload>(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
		...
		int nestedImageDepth)
	{
		...
		if (allowedElements is null) throw new ArgumentNullException(nameof(allowedElements));
		if (allowedAttributes is null) throw new ArgumentNullException(nameof(allowedAttributes));
