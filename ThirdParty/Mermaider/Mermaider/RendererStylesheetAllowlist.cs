...
internal static partial class RendererStylesheetAllowlist
{
	...
	private static Regex QuotedFontPattern() => Net48QuotedFontPatternRegexCache.Value;
	private static class Net48QuotedFontPatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^'(?:[A-Za-z0-9 _-]|\\[0-9A-F]{1,4} )*'$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex FontSizePattern() => Net48FontSizePatternRegexCache.Value;
	private static class Net48FontSizePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(?:0|[1-9][0-9]*)(?:\.[0-9]{1,3})?(?:px|rem|em|%)$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StrictShapeRulePattern() => Net48StrictShapeRulePatternRegexCache.Value;
	private static class Net48StrictShapeRulePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(?<indent> {2}| {4})(?<selector>\.cls-[A-Za-z_][A-Za-z0-9_-]*) rect, \k<selector> polygon, \k<selector> circle, \k<selector> ellipse \{ fill: (?<fill>#[0-9A-Fa-f]{3,8}); stroke: (?<stroke>#[0-9A-Fa-f]{3,8}); \}$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}

	private static Regex StrictTextRulePattern() => Net48StrictTextRulePatternRegexCache.Value;
	private static class Net48StrictTextRulePatternRegexCache
	{
		internal static readonly Regex Value = new Regex(@"^(?<indent> {2}| {4})(?<selector>\.cls-[A-Za-z_][A-Za-z0-9_-]*) text \{ fill: (?<fill>#[0-9A-Fa-f]{3,8}); \}$", RegexOptions.None | RegexOptions.Compiled, TimeSpan.FromMilliseconds(TimeoutMs));
	}
