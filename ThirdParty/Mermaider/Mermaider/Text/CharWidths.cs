...
{
	private const string NarrowCharacters = "iltfjI1!|.,:;'";

	private const string WideCharacters = "Wwm@%";

	private const string SemiNarrowCharacters = "()[]{}\\/-\"`";
	...
	internal static double GetCharWidth(char c)
	{
		...
		if (WideCharacters.IndexOf(c) >= 0)
			return 1.2;

		if (NarrowCharacters.IndexOf(c) >= 0)
			return 0.4;

		if (SemiNarrowCharacters.IndexOf(c) >= 0)
			return 0.5;
